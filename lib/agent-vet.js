...
function auditTools(tools) {
  const movesValue = [], callerSigned = [], namedButNoSurface = [], wantsSecret = [], readOnly = [];
  for (const t of tools || []) {
    ...
    const required = new Set((Array.isArray(schema.required) ? schema.required : []).flatMap(tokenize));
    ...
    if (gate) callerSigned.push({ name, verb, field: valueField, recipientField: recipient || null, gate });
    else movesValue.push({ name, verb, field: valueField, recipientField: recipient || null });
  }
  return { movesValue, callerSigned, namedButNoSurface, wantsSecret, readOnly };
}
...
async function vetAgent({ url, payTo, chain = 'base', knownBadScreen = null, screenFn = null } = {}) {
  ...
  let why = '';
  if (url) {
    const named = ((s && s.callerSigned) || []).length + ((s && s.namedButNoSurface) || []).length;
