 ...
 *
 * ⚠️ UN NOM MAL DECOUPE DEVIENT `readOnly` — mesure du 2026-07-31 par `auditTools`, a schema IDENTIQUE
 * (`amount` + `to`, tous deux REQUIS, soit une surface de paiement sans ambiguite). Seul le nom change:
 *   send_usdc ....... movesValue   ✓
 *   swap1inch ....... readOnly     ⛔  le chiffre n'etait pas une frontiere: ["swap1inch"]
 *   send2wallet ..... readOnly     ⛔  idem
 *   USDCSend ........ readOnly     ⛔  la regle ne coupait que minuscule->MAJUSCULE, jamais MAJ->MAJmin
 *   get_price ....... readOnly     ✓  le seul des cinq qui merite ce mot
 * Quatre outils qui bougent des fonds recevaient donc le mot rendu a un lecteur de prix. Le verbe est la
 * moitie de la regle a deux conditions de ce module: ne pas savoir le LIRE, c'est repondre « inerte » sur
 * une surface qu'on n'a simplement pas su nommer — une lecture ratee rendue comme un constat.
 *
 * Les sous-decoupes sont AJOUTEES, jamais substituees: `x402-settle` doit garder le token `x402`, qui est un
 * nom de protocole, tout en gagnant `x` et `402`. Le sur-ensemble est sur ici parce que les trois
 * consommateurs n'interrogent que l'APPARTENANCE — un token de plus peut faire TROUVER un verbe, jamais en
 * perdre un. Et les morceaux naissent d'une frontiere reelle, donc aucune sous-chaine n'est inventee.
 *
 * ⚠️ CE QUE CA NE REPARE PAS, et qui reste faux: `sendtoken` et `SENDTOKEN` rendent toujours UN seul token.
 * Sans frontiere — ni casse, ni chiffre, ni separateur — il n'y a rien a lire, et deviner par recherche de
 * sous-chaine ferait matcher `send` dans `resendable`. La limite est reelle, donc elle se dit plutot que de
 * se combler au jugé; `DISCLOSURE` la porte.
 */
function tokenize(s) {
  const base = String(s || '')
    .replace(/([A-Z]+)([A-Z][a-z])/g, '$1 $2')   // acronyme colle a un mot: USDCSend -> USDC Send
    .replace(/([a-z0-9])([A-Z])/g, '$1 $2')      // camelCase ordinaire: sendETH -> send ETH
    .split(/[^A-Za-z0-9]+/)
    .filter(Boolean)
    .map((w) => w.toLowerCase());
  const out = [];
  const ajoute = (w) => { if (w && !out.includes(w)) out.push(w); };
  for (const w of base) {
    ajoute(w);
    // Un chiffre au milieu d'un nom est une frontiere de mot autant qu'un `_`. Mais il appartient aussi a
    // certains noms entiers (`x402`, `1inch`), d'ou le tout ET les morceaux.
    if (/[a-z][0-9]|[0-9][a-z]/.test(w)) for (const part of w.match(/[a-z]+|[0-9]+/g) || []) ajoute(part);
  }
  return out;
}

const httpPlain = require('node:http');

/**
 * ⚠️ THREE WAYS OF NEVER LEAVING THE PROCESS, ALL REPORTED AS "the endpoint did not answer at all".
 * Measured 2026-07-29:
 *
 *   the URL's PORT was never passed to https.request. `https://host:8443/mcp` was probed on 443.
 *     Proved with a bare TCP listener: ZERO connections arrived on the requested port. A self-hosted
 *     MCP server — which is to say most of them, on :3000, :8080, :8443 — was audited as absent.
 *   the module was hardcoded to https, so an `http://` endpoint could never be reached either.
 *   `new URL(url)` threw for a malformed string, and the throw escaped `post` entirely.
 *
 * All three collapsed into `resolve(null)` → `reachable: false, 'did not answer at all'`. On a tool
 * whose whole job is to report what an agent endpoint exposes, that is the worst possible direction:
 * "I never sent the request" was published as "there is nothing there".
 *
 * `probed` now separates them. A rejected input says NOTHING about the endpoint and must never be read
 * as evidence about it; a real timeout does. (Checked before fixing: no surveyed endpoint in this
 * repo's data uses a non-standard port, so the published 62%-unauditable figure is NOT affected.)
 */
const post = (url, body, timeout = 12000) => new Promise((resolve) => {
  const data = JSON.stringify(body);
  let u;
  try { u = new URL(url); } catch { return resolve({ probed: false, badInput: 'not a parseable URL' }); }
  const mod = u.protocol === 'https:' ? https : u.protocol === 'http:' ? httpPlain : null;
  if (!mod) return resolve({ probed: false, badInput: 'scheme "' + u.protocol + '" is not http(s)' });
...
/* ⚠️ LA PREMIERE TRAME N'EST PAS FORCEMENT LA REPONSE. L'ancienne version prenait la premiere ligne
 * `data:` et s'arretait la. Or un serveur MCP en streaming envoie couramment une notification (ou un
 * message de progression) AVANT sa reponse. Cette notification est du JSON parfaitement valide, donc
 * aucun repli ne se declenchait: on rendait la notification, elle n'a pas de `result.tools`, et un
 * serveur qui avait parfaitement repondu ressortait « the surface is UNREAD ».
 *
 * Mesure du 2026-07-29 contre un vrai serveur local, six formes: JSON nu, `data:` simple, `event:`
 * nomme, commentaire de keep-alive et CRLF passaient tous — seule la reponse en DEUX TRAMES echouait.
 * Sur un outil qui recense l'auditabilite des endpoints publics, ce faux negatif GONFLE le compte des
 * non-auditables, et le streaming est le mode normal d'un MCP moderne.
 *
 * On lit donc TOUTES les trames, on concatene les `data:` consecutifs (c'est ce que la spec SSE exige
 * pour un JSON coupe en plusieurs lignes), et on retient la premiere qui est une REPONSE — `result` ou
 * `error`. A defaut, on rend quand meme la premiere trame lisible: un serveur qui n'a envoye que des
 * notifications parle MCP, et le dire « pas MCP » serait l'accusation inverse. */
function parseMcp(raw) {
  if (!raw) return null;
  try { return JSON.parse(raw); } catch { /* pas du JSON nu: c'est peut-etre du SSE */ }

  const trames = [];
  let courante = [];
  for (const l of String(raw).split(/\r?\n/)) {
    if (l.startsWith('data:')) courante.push(l.slice(5).trim());
    else if (!l.trim() && courante.length) { trames.push(courante.join('')); courante = []; }
  }
  if (courante.length) trames.push(courante.join(''));

  let premiere = null;
  for (const t of trames) {
    let j; try { j = JSON.parse(t); } catch { continue; }
    if (premiere === null) premiere = j;
    if (j && (j.result !== undefined || j.error !== undefined)) return j;   // une REPONSE, pas une notification
  }
  return premiere;
}
...
async function introspectHttp(url) {
  ...
  /* Une entree REFUSEE ne dit rien sur l'endpoint — et surtout pas qu'il est absent. `probed:false`
   * remonte jusqu'ici pour que l'appelant ne puisse pas confondre « je n'ai pas envoye la requete »
   * avec « personne n'a repondu ». Les deux valaient `reachable:false` avec la meme phrase. */
  if (init && init.probed === false) {
    return { reachable: false, probed: false, reason: 'REJECTED INPUT: ' + init.badInput
      + ' — nothing was contacted, so this says NOTHING about the endpoint. Fix the URL and re-run.' };
  }
  if (!init) return { reachable: false, probed: true, reason: 'no response — the endpoint did not answer at all' };
  ...
  // `answered` = an HTTP response came back, whatever its status. The comment above already established
  // that 401/403 are presence, not absence — the same is true of 404, 500, 302 and 405. Not generalising
  // it left every one of those reported as "the endpoint did not answer", which is simply false: it did.
  if (init.status !== 200) return { reachable: false, probed: true, answered: true,
    status: init.status, reason: 'HTTP ' + init.status + ' — the host answered, so something is running there; it is this path that is not an auditable MCP endpoint' };

  /* ⚠️ QUATRE SITUATIONS SANS RAPPORT RENDAIENT LA MEME PHRASE. Mesure du 2026-07-29 contre un vrai
   * serveur local — « answered initialize but returned no tool list » sortait pour:
   *
   *   une page HTML de login           (ce n'est PAS un serveur MCP)
   *   une erreur JSON-RPC explicite    (il a REFUSE la question)
   *   un `result` sans `tools`         (serveur MCP, pas de liste)
   *   un vrai serverInfo sans outils   (serveur MCP, zero outil)
   *
   * Sur un outil dont la sortie sert a decider si on paie un agent, « ce n'est pas un endpoint MCP » ne
   * doit pas se lire « un endpoint MCP qui n'expose rien ». Et `tools: []` est une REPONSE — un serveur
   * qui declare zero outil a dit quelque chose; `|| null` l'ecrasait sur « pas de liste ». */
  const parsedInit = parseMcp(init.body);
  const base = { reachable: true, probed: true, answered: true,
    serverInfo: (parsedInit && parsedInit.result && parsedInit.result.serverInfo) || null };
  if (!parsedInit) return { ...base, mcp: false, tools: null,
    reason: 'answered HTTP 200 but the body is not an MCP reply (neither JSON nor an SSE data frame) — this is very likely not an MCP endpoint at all' };

  const list = await post(url, { jsonrpc: '2.0', id: 2, method: 'tools/list' });
  if (list && list.probed === false) return { ...base, mcp: true, tools: null,
    reason: 'REJECTED INPUT on the tools/list call: ' + list.badInput };
  if (!list) return { ...base, mcp: true, tools: null,
    reason: 'answered initialize, then did not answer tools/list at all — the surface is UNREAD, not empty' };
  const parsedList = parseMcp(list.body);
  if (!parsedList) return { ...base, mcp: true, tools: null,
    reason: 'answered tools/list with something that does not parse as MCP — the surface is UNREAD, not empty' };
  if (parsedList.error) return { ...base, mcp: true, tools: null,
    reason: 'REFUSED tools/list (' + (parsedList.error.message || parsedList.error.code) + ') — it declined to say what it exposes, which is not the same as exposing nothing' };

  const tools = parsedList.result && Array.isArray(parsedList.result.tools) ? parsedList.result.tools : null;
  return { ...base, mcp: true, tools,
    reason: tools ? null : 'answered tools/list without a `tools` array — the surface is UNREAD, not empty' };
}
...
async function vetAgent({ url, payTo, chain = 'base', knownBadScreen = null, screenFn = null,
  ...
  if (url && !live.reachable) {
    // Ne pas dire « n'a pas repondu » quand il a repondu. Un 404 et un port ferme menent tous deux a
    // « on ne peut pas auditer », mais ce sont deux faits differents, et le second seul est une absence.
    return { ...out, verdict: 'unreachable',
      reason: (live.probed === false
        ? live.reason
        : live.answered
          ? 'the endpoint answered but not as an auditable MCP service (' + live.reason + '). Something is running at that host; this path is not it.'
          : 'the endpoint did not answer (' + live.reason + ').')
        + ' A listing is not a service, and paying one that does not respond is the simplest loss available.',
  ...
  if (payTo) {
    ...
    /* `screen.js` refuse d'appeler son zero un verdict propre: sans liste chargee il rend
     * `{blocked:false, available:false}`. Un booleen `knownBad` ne peut pas porter cette distinction, donc
     * « pas sur la liste » et « la liste n'a pas ete lue » sortaient identiques. Le cas est celui de la
     * PANNE: `loadFloor()` rend un objet truthy a `available:false` quand le fichier manque — le verdict
     * devient faussement rassurant au moment precis ou le plancher casse.
     *
     * `knownBad` reste inchange pour les appelants existants: son `true` n'a jamais menti, seul son
     * `false` recouvrait deux choses. */
    const screenState = !screened ? 'not_run'
      : (screened.available === false ? 'unavailable' : (screened.blocked ? 'blocked' : 'clear'));

    out.payment = {
      knownBad: !!(screened && screened.blocked),
      screen: screenState,
      screenReason: screened ? screened.reason
        : 'no known-bad list was supplied to vetAgent (knownBadScreen/screenFn absent) — this payee was '
          + 'NOT screened, which is a gap on our side and says nothing about the address',
  ...
  /* UN CRIBLE QUI N'A PAS TOURNE SE DIT. Il ne fait PAS basculer le verdict — un beneficiaire non crible
   * n'est pas un mauvais beneficiaire, et refuser ici rendrait vetAgent inutilisable sans plancher, ce
   * qui est le fail-closed pousse trop loin. Mais la sortie ne doit pas laisser croire qu'il a ete
   * examine et blanchi. La note voyage avec TOUS les verdicts suivants, y compris les propres. */
  if (p && (p.screen === 'not_run' || p.screen === 'unavailable')) {
    out.unscreened = 'the payee was NOT checked against the known-bad list (' + p.screen + '): '
      + p.screenReason + '. Every verdict below is composed WITHOUT that check.';
  }
...
const DISCLOSURE = 'This never returns "safe". It deliberately does NOT grade how good the description reads: ' +
  ...
  'and never calls a tool. One limit worth knowing when you read `readOnly`: a value verb is found by ' +
  'splitting the tool NAME into words, so a name that carries no word boundary at all — `sendtoken`, ' +
  'all one case and no separator — cannot be split and lands in `readOnly` however it behaves. Underscores, ' +
  'hyphens, camelCase, acronyms and digits are all read as boundaries; nothing else can be. `readOnly` here ' +
  'means no value verb was READABLE in the name, not that the tool was shown to be inert.';
