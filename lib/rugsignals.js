...
function assessRugFields(r, sim) {
  ...
  for (const [on, msg] of gated) {
    if (!on) continue;
    if (live) armed.push(msg); else flags.push('(defused by renounced ownership) ' + msg);
  }
  ...
  if (lpLocked == null) unknowns.push('LP lock status');
    ...
    const pct = Math.round(lpLocked * 100);
    flags.push('only ' + pct + '% of liquidity is locked or burned — the rest can be withdrawn at will');
  }
  ...
  return { verdict, reason, armed, flags, unknowns, ownerLive: live, owner: r.owner_address || null,
