 ...
 */
/* ⚠️ `calendar.google.com` MANQUAIT — trouve en servant pour de vrai, le 2026-07-29. Un compte Base
 * officiel a envoye un lien `calendar.app.google` pour reserver un creneau; ce module a repondu
 * « Domain "app.google" is not a platform this recognises ». Or la liste contenait deja `docs.google.com`
 * et `drive.google.com`, et sa toute premiere entree est `calendly.com` — le meme usage exactement.
 * L'omission etait un oubli, pas une decision, et elle transformait le service de rendez-vous le PLUS
 * courant en signal. `calendar.app.google` est le raccourcisseur officiel de Google pour ces pages, d'ou
 * les deux formes. Un outil qui crie sur Google Agenda se fait desinstaller — c'est la these de cette
 * liste, appliquee contre elle-meme. */
const LEGIT_NEUTRAL = ['calendly.com', 'cal.com', 'savvycal.com', 'hubspot.com', 'notion.so', 'notion.site',
  'docs.google.com', 'drive.google.com', 'calendar.google.com', 'app.google', 'meetings.hubspot.com',
  'typeform.com', 'airtable.com', 'lu.ma', 'eventbrite.com'];
 ...
 */
/** A raw address is not a domain. `185.62.188.12` has no registrable part to report, and reporting one is
 *  how "188.12" ended up looking like an answer. IPv6 arrives from `new URL` still bracketed. */
...
function checkLink(url) {
  ...
  /* ⚠️ EVERYTHING BELOW ANSWERS A QUESTION ABOUT A DOMAIN, so a URL that HAS no domain must never reach it.
   * Measured 2026-07-29: `file:///C:/x/setup.exe`, `javascript:alert(1)` and `data:text/html;…` all parse,
   * all yield `hostname: ''`, and all came back as
   *
   *   verdict `unrecognised` — 'Domain "" is not a platform this recognises. That is not an accusation'
   *
   * An empty host read exactly like an unremarkable domain. On `file:` pointing at an .exe that sentence is
   * actively wrong: the link IS the install ask this module exists to name. The scheme is the answer here,
   * not the host, so it gets its own verdict rather than a domain verdict about a domain that is not there. */
  const scheme = u.protocol.replace(/:$/, '').toLowerCase();
  if (scheme !== 'http' && scheme !== 'https') {
    return { ok: true, url, hostname: u.hostname || null, registrable: null, impersonates: [], browserNative: false,
      scheme, verdict: 'not_a_web_link',
      reason: 'This is not a web address — its scheme is "' + scheme + ':", not http(s). A link handed to you in '
        + 'a message is expected to open a page; this one does something else' + (scheme === 'file'
          ? ' (it points at a file on the machine that opens it — that IS the install, not a link to one).'
          : scheme === 'javascript' || scheme === 'data'
            ? ' (it carries its own executable content instead of pointing anywhere).'
            : '.') + ' No domain check applies, because there is no domain to check.' };
  }

  const host = u.hostname.toLowerCase();
  if (isIpLiteral(host)) {
    return { ok: true, url, hostname: host, registrable: null, impersonates: [], browserNative: false,
      verdict: 'ip_literal',
      reason: 'This points at a raw address (' + host + '), not a domain. Nobody had to register anything, so '
        + 'there is no owner to look up and nothing a name could vouch for.' };
  }
  ...
  const owns = (b) => IMPERSONATED[b].some((owned) => reg === owned || reg.endsWith('.' + owned));
  const impersonates = Object.keys(IMPERSONATED).filter((b) =>
    prefixLabels.some((l) => l === b || l.includes(b)) && !owns(b) && !reg.startsWith(b + '.'));

  /* ⚠️ THE SAME TRICK, ONE CHARACTER EARLIER — and this module's own thesis names it: "anyone can put any
   * brand name to the LEFT of a domain they control". Before a dot, it was caught. Before an `@`, it was not:
   *
   *   https://zoom.us@evil-recorder.com/join  ->  `unrecognised`, "That is not an accusation"
   *
   * `new URL` files everything before the `@` under `username`/`password`, so `hostname` is clean and every
   * check above passes. The browser goes to evil-recorder.com; a human skimming the message reads "zoom.us".
   * It is the more deceptive of the two forms, and it was scoring lower: `brand_impersonation` is FATAL in
   * `vetApproach` (verdict `fraud`) while `unrecognised` is one flag among several. */
  const userLabels = (String(u.username || '') + '.' + String(u.password || ''))
    .toLowerCase().split(/[^a-z0-9]+/).filter(Boolean);
  const impersonatesInUserinfo = Object.keys(IMPERSONATED).filter((b) =>
    userLabels.some((l) => l === b || l.includes(b)) && !owns(b));

  const browserNative = BROWSER_NATIVE.some((d) => reg === d || host === d || host.endsWith('.' + d));

  if (impersonatesInUserinfo.length) return { ok: true, url, hostname: host, registrable: reg,
    impersonates: impersonatesInUserinfo, browserNative: false, viaUserinfo: true,
    verdict: 'brand_impersonation',
    reason: 'This link is NOT ' + impersonatesInUserinfo[0] + '. Everything before the "@" is a username, not a '
      + 'destination — your browser ignores it and goes to "' + reg + '". Putting a brand there is the same move '
      + 'as putting it in a subdomain, one character earlier, and it reads even more convincingly.' };
...
function vetApproach({ links = [], platform, message, asksToInstall, asksForKeyOrSeed, asksForSignature,
  ...
  /* A link whose scheme CARRIES content instead of pointing at a page. There is no legitimate inbound use:
   * `javascript:` and `data:` execute in your context, `file:` names something already on your disk. This is
   * the install ask expressed as a link, so it is graded like the install ask. `mailto:`/`tel:` are ordinary
   * and deliberately absent — a check that fires on an email address gets muted. */
  const carrier = linkChecks.find((l) => l.verdict === 'not_a_web_link'
    && ['javascript', 'data', 'file', 'vbscript', 'blob'].includes(l.scheme));
  if (carrier) fatal.push('A "link" here is a ' + carrier.scheme + ': URL — it does not point at a page, it '
    + 'carries or names content directly. That is the install, written as a link.');

  const flags = [];
  if (linkChecks.some((l) => l.verdict === 'ip_literal')) flags.push('A link points at a raw IP address rather '
    + 'than a domain. Nothing was registered, so there is no name for anyone to have vouched for.');
  /* ⚠️ CE DRAPEAU DEMANDE « est-ce un outil d'ENREGISTREMENT ? » A TOUTE plateforme nommee. Mesure du
   * 2026-07-29 sur une vraie approche: `platform: 'Google Agenda'` — nomme comme outil de RENDEZ-VOUS —
   * ressortait « is not one of the browser-based tools productions actually record on ». C'est vrai et
   * hors sujet: personne n'a pretendu enregistrer dessus. Un drapeau qui a raison sur les mots et tort
   * sur le fond est du bruit, et le bruit apprend a sauter la ligne.
   * Une plateforme qui figure parmi les services NEUTRES reconnus (agenda, formulaire, doc) n'est donc
   * plus signalee — elle ne prouve toujours rien, ce que `recognised_neutral` dit deja par ailleurs. */
  const platformeNeutre = platform && LEGIT_NEUTRAL.some((d) =>
    String(platform).toLowerCase().replace(/\s+/g, '').includes(d.split('.')[0]));
  if (platform && !platformeNeutre && !BROWSER_NATIVE.some((d) => String(platform).toLowerCase().includes(d.split('.')[0]))) {
