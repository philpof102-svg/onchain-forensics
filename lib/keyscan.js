 ...
 */
/* Un keystore Web3 porte TOUJOURS le mot `ciphertext`. Ce filtre coute une recherche de sous-chaine et
 * ne perd rien — c'est ce que le plafond de taille cherchait vraiment a faire (eviter de parser des
 * fichiers qui ne peuvent pas etre des keystores), sauf qu'un plafond de taille jette AUSSI ceux qui
 * peuvent l'etre. */
const PARSE_CEILING = 4 * 1024 * 1024;   // meme borne que le balayage de chemins, qui la DIVULGUE deja

function readKeystore(text) {
  if (text.indexOf('ciphertext') < 0) return null;     // pas un keystore, et prouve sans rien parser
  /* ⚠️ ICI VIVAIT `if (text.length > 200000) return null` — UN REFUS DE REGARDER, PUBLIE COMME UNE
   * ABSENCE. Mesure du 2026-07-29: le MEME keystore valide, place dans un fichier de 200 458 octets,
   * rendait `null`, et `scanKeyText` sur ce fichier rendait AUCUNE TROUVAILLE — pas une trouvaille
   * faible, zero. Sur un outil dont la sortie EST un rapport d'exposition, « je n'ai pas ouvert ce
   * fichier » se lisait « ce fichier est propre ». Un keystore est petit, mais un fichier QUI EN
   * CONTIENT UN ne l'est pas forcement: un dump, un export de portefeuille colle a un log, un JSON
   * d'application qui embarque le coffre.
   *
   * Et le module savait deja faire: 270 lignes plus bas, `if (st.size > 4 Mo) { skipped.tooBig++; }`
   * COMPTE ce qu'il saute, et ce compte remonte a l'appelant. Le meme probleme, resolu correctement a
   * un endroit et perdu en silence a l'autre. C'est desormais la meme borne, et elle se declare. */
  if (text.length > PARSE_CEILING) return { unchecked: 'file is larger than ' + PARSE_CEILING + ' bytes', bytes: text.length };
  let j;
...
function scanKeyText(text, { filename = '' } = {}) {
  ...
  /* Un fichier qu'on n'a PAS examine ne doit pas sortir de ce scan silencieusement. Il porte le mot
   * `ciphertext` — donc il ressemble a du materiel de cle — et on a refuse de le parser. C'est une
   * trouvaille a part entiere, distincte de `keystore_file` (examine, c'en est un) et de l'absence de
   * trouvaille (examine, ce n'en est pas un). */
  if (ks && ks.unchecked) {
    findings.push({ line: 1, verdict: 'keystore_unchecked', bytes: ks.bytes,
      note: 'This file contains the word "ciphertext" — it may well be a wallet keystore — but it was NOT '
        + 'examined: ' + ks.unchecked + '. This is not a clean result. Nothing was read here, so treat this '
        + 'file as unknown rather than safe, and open it yourself.' });
    return findings;
  }
  if (ks) {
...
function scanKeyPaths(paths, { maxFiles = 40000 } = {}) {
  /* ⚠️ MEME DEFAUT QUE `scanPaths`, MEME CHEMIN D'APPEL — la garde est PARTAGEE, pas recopiee, pour qu'il
   * n'y ait pas deux definitions de « liste de chemins utilisable » qui divergent. Voir la note complete
   * sur `exigeListeChemins` dans lib/seedscan.js: une chaine est iterable, donc elle se decoupe en chemins
   * d'un caractere et le balayage part sur la racine du disque. */
  require('./seedscan').exigeListeChemins(paths);
  ...
  /* ⚠️ DEUX DEFAUTS MESURES ICI LE 2026-07-28, dans le regroupement lui-meme.
   *
   *   1. LE LECTEUR N'AVAIT PAS SUIVI SON ECRIVAIN. Le correctif multiligne pose la valeur sur
   *      `valueLine`; `line` porte l'ETIQUETTE. Le calcul d'empreinte relisait `line`, donc n'y trouvait
   *      aucun hex, donc n'identifiait AUCUN secret trouve en JSON/YAML indente — la forme la plus banale.
   *
   *   2. `'unknown'` ETAIT UNE CLE DE REGROUPEMENT. Tous les echecs d'identification tombaient donc dans
   *      le meme seau, et « je n'ai pas pu identifier ces valeurs » ressortait exactement comme « c'est la
   *      MEME valeur ». Or ce regroupement existe precisement pour repondre a cette question-la.
   *
   * Ensemble, sur deux fichiers portant deux cles DIFFERENTES en JSON indente, mesure avant correctif:
   *   [{ fingerprint: 'unknown', liveLocations: [a.json:2, b.json:2], distinctFiles: 2 }]
   * soit UN secret partage par deux fichiers, la ou il y en a DEUX. Sur un scanner d'exposition, ce
   * sous-comptage est un faux feu vert: on annonce moins de cles dehors qu'il n'y en a.
   *
   * Forme retenue, la meme que `findVaults`: l'echec devient une ENTREE avec `fingerprint: null` et une
   * raison, jamais un identifiant partage — et chaque trouvaille non identifiable garde son propre seau. */
  const groups = new Map();
  for (const f of findings) {
    if (f.verdict !== 'cleartext_key' && f.verdict !== 'retained_copy') continue;
    const noLigne = f.valueLine || f.line;              // multiligne: la valeur n'est pas sur la ligne de l'etiquette
    let fp = null, raison = null;
    try {
      ...
      else raison = 'no 64-hex value was found on that line when it was re-read — the file changed, or this '
        + 'finding cannot be identified. It is listed on its own rather than merged with other unidentified '
        + 'findings, which would have read as "the same secret".';
    } catch (e) {
      raison = 'the file could not be re-read (' + (e && e.code || 'unknown') + '), so this value was never '
        + 'identified. It is NOT known to be the same secret as any other row.';
    }
    // Pas d'empreinte ⇒ seau prive. La cle porte un prefixe impossible pour un sha256 tronque.
    const cle = fp || 'nofp:' + f.file + ':' + f.line;
    if (!groups.has(cle)) groups.set(cle, { fingerprint: fp, raison, live: [], retained: [] });
    groups.get(cle)[f.verdict === 'retained_copy' ? 'retained' : 'live'].push(f.file + ':' + f.line);
  }

  const secrets = [...groups.values()].map((g) => ({
    fingerprint: g.fingerprint,
    ...(g.raison ? { unfingerprinted: g.raison } : {}),
  ...
  /* ⚠️ MEME DEFAUT QUE `scanPaths`, MEME CHEMIN D'APPEL — corrige le 2026-07-30 en meme temps.
   *
   * Les deux outils (`seed_exposure`, `key_exposure`) prennent leurs chemins de `SEED.defaultPaths()`. Avec
   * une liste vide — ce que cette fonction rend desormais quand aucune racine n'est resolue — ce bloc rendait
   * { scanned: 0, complete: true, verdict: 'no_cleartext_key' }: zero fichier ouvert, et le mot qui rassure
   * le plus. C'est le piege du correctif qui devient un defaut: rendre `[]` en amont sans apprendre a
   * l'aval a le LIRE aurait fabrique une affirmation plus forte que celle qu'on venait de retirer.
   *
   * `no_cleartext_key` dit « j'ai lu et il n'y en avait pas ». `not_scanned` dit « je n'ai pas lu ». */
  const rienLu = scanned === 0;
  return {
    scanned, bytes, skipped, complete: totalSkipped === 0 && !rienLu,
    coverage: rienLu
      ? 'none — NOT ONE FILE WAS OPENED, so this run says nothing at all about your keys. '
        + paths.length + ' path(s) were given and ' + totalSkipped + ' read(s) were skipped. Note that '
        + '`vaults` is still reported: it answers a different question (presence on disk) and does not '
        + 'depend on these paths. Pass the paths you mean explicitly.'
      : 'read — ' + scanned + ' file(s) were opened' + (totalSkipped
        ? ', and ' + totalSkipped + ' were skipped (see `skipped`); a key could sit in any of those.'
        : ' and nothing was skipped.'),
    secrets, keystores, vaults: findVaults(),
    verdict: liveKeys ? 'cleartext_keys_present' : secrets.length ? 'only_retained_copies'
      : rienLu ? 'not_scanned' : 'no_cleartext_key',
