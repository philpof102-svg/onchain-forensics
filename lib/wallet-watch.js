 ...
 */
/* `lireJson` — joint d'injection, present dans biii et ABSENT ici jusqu'au 2026-07-28. C'est une derive
 * qu'aucune comparaison de proprietes ne pouvait voir: elle porte sur la SIGNATURE, pas sur la sortie.
 * Trouvee parce qu'un cas de parite a rougi sur « Protocol http: not supported » — la copie appelait le
 * vrai reseau la ou l'autre acceptait un bouchon. Sans ce joint, cette fonction est intestable hors ligne,
 * ce qui explique qu'elle n'ait ete nommee dans aucun test de ce depot. */
async function judgeCounterparty(exp, address, lireJson) {
  ...
  /* ═══ UN CHAMP ABSENT N'EST PAS UN CHAMP A `false` ═══
   * `if (info.is_contract)` confondait « l'explorateur dit non » et « l'explorateur n'a rien dit ». Mesure
   * du 2026-07-28: `{is_contract:false}` et `{}` rendaient tous deux « a plain wallet, not a contract »,
   * severite null — une reponse 200 a la forme changee faisait AFFIRMER la nature d'une adresse que
   * personne n'avait mesuree, et c'est la lecture rassurante qui sortait.
   *
   * On divulgue sans escalader: « je n'ai pas pu lire » n'est pas « contrat non verifie ».
   *
   * ⚠️ C'est le `=== false` qui porte la distinction, pas le `=== true` (verifie par mutation: le second
   * est redondant, un undefined etant falsy). Ne pas « simplifier » le `=== false` en `else`. */
  if (info.is_contract === true) {
    if (info.is_verified === true) {
      notes.push('verified contract' + (info.name ? ' (' + info.name + ')' : ''));
    } else if (info.is_verified === false) {
      notes.push('UNVERIFIED contract — its source is not published');
      if (severity !== 'high') severity = 'medium';
    } else {
      notes.push('a contract, but the explorer did not say whether its source is verified — that is a gap '
        + 'in what we were SENT, not a finding about the contract');
    }
  } else if (info.is_contract === false) {
    notes.push('a plain wallet, not a contract');
  } else {
    notes.push('the explorer answered but did not say whether this address is a contract, so nothing here '
      + 'says what it is — this is NOT "a plain wallet"');
  }
...
/**
 * ⚠️ THIS FILE'S OWN HEADER SAYS "three outcomes, never two: a check that could not run is reported as
 * such and never as 'nothing found', because on a wallet monitor a silent failure reads as 'you are safe'."
 * The state reader was the one place that did not obey it. `catch { return null }` folded four situations
 * into two. Measured 2026-07-29:
 *
 *   file ABSENT (genuine first run)        -> null   -> firstRun true    ✓ correct
 *   file VALID                             -> object -> firstRun false   ✓ correct
 *   file TRUNCATED (crash mid-write)       -> null   -> firstRun true    ✗
 *   file EMPTY (disk full)                 -> null   -> firstRun true    ✗
 *   file of NUL bytes (fs corruption)      -> null   -> firstRun true    ✗
 *
 * And `firstRun` does not make the guard fail — it makes it SILENT while still returning `ok: true`.
 * Every standing approval is relabelled "inventory rather than an event", and
 * `if (known.counterparties.has(to) || firstRun) continue` skips judging EVERY counterparty, including an
 * address that has never been seen receiving money. A corrupt byte turns the monitor off.
 *
 * It is the same lesson this module already learned about the approval sweep sixty lines below — do not
 * replace the reference on an INCOMPLETE read — applied to the reference itself.
 *
 * Contract kept deliberately backward-compatible: absent still returns `null`, because that is what a
 * first run genuinely is. Only the unreadable cases throw, and `watchWallet` turns that into an
 * `unavailable` line rather than a crash.
 */
const readState = (chain, owner) => {
  const p = statePath(chain, owner);
  let raw;
  try {
    raw = fs.readFileSync(p, 'utf8');
  } catch (e) {
    if (e.code === 'ENOENT') return null;          // never written: a real first run
    throw new Error('wallet-watch state exists but could not be read (' + e.code + ') at ' + p
      + ' — refusing to treat it as a first run, which would silence every alert.');
  }
  try {
    return JSON.parse(raw);
  } catch {
    throw new Error('wallet-watch state at ' + p + ' is present but unparseable (' + raw.length
      + ' bytes) — refusing to treat it as a first run, which would silence every alert.');
  }
};
...
async function watchWallet(chain, owner, { persist = true } = {}) {
  ...
  /* An unreadable state is NOT a first run. Losing the reference means we can no longer tell a new door
   * from an old one — so we say so, and we keep judging. Judging everything as if unknown is noisy;
   * judging nothing because the memory is gone is the failure this whole module exists to prevent. */
  let prev = null, stateLost = null;
  try { prev = readState(chain, owner); }
  catch (e) { stateLost = e.message; }

  const firstRun = !prev && !stateLost;
  const known = {
  ...
  if (stateLost) unavailable.push('the stored state for this wallet could not be read, so nothing below can '
    + 'be called NEW with confidence — this run is being judged without a memory. ' + stateLost);
  ...
  if (!ap.ok) {
    ...
    for (const l of ap.live) {
      ...
      alerts.push({
        ...
        judgment: j.notes,
        // The `why` is a claim about TIME, and it is only defensible when we actually held the memory it
        // rests on. "Someone granted it since" names a third party's act; saying it from a lost state
        // would be inventing an event out of our own forgetting.
        why: firstRun
          ? 'First run, so this is inventory rather than an event — it is what already existed.'
          : stateLost
            ? 'We could not read what we knew before this run, so we cannot say whether this is new. Reported as unknown rather than assumed old — and NOT as "someone granted it since", which we have no basis for.'
  ...
  if (!txs || !Array.isArray(txs.items)) {
    ...
    for (const t of txs.items) {
      ...
      alerts.push({
        ...
        judgment: j.notes,
        why: stateLost
          ? 'We could not read what we knew before this run, so we cannot say this address is a first — it is reported because it could not be ruled out.'
          : 'This address had never appeared as a destination from this wallet before ' + (t.timestamp || 'now') + '.',
      });
    }
  }
  ...
  if (persist) {
    ...
    /* ⚠️ THE WRITER WAS MANUFACTURING THE FAILURE THE READER SWALLOWED. A direct `writeFileSync` is not
     * atomic: interrupt it — a crash, a kill, a full volume (which this project has already lived through)
     * — and what is left on disk is a HALF-WRITTEN JSON file. That is exactly the truncated state the
     * reader above used to report as "no state, first run". The two halves are each harmless; together
     * they are a switch that turns the monitor off quietly.
     * Write beside it, then rename: on the same filesystem the rename is atomic, so a reader sees either
     * the whole old state or the whole new one, never a fragment. */
    const dest = statePath(chain, owner);
