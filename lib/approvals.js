...
const { allowancesBatch } = require('./multicall');
 ...
 * into a real one.
 */
/* TROIS VALEURS, PARCE QUE L'APPELANT EN ATTEND TROIS. checkApprovals trie deja sur `null` (non verifie),
 * `0n` (revoque) et `false` (revert: « rien ici », definitif — voir le tri plus bas dans ce fichier). Le
 * repli solo rendait `null` pour tout ce qui n'etait pas un nombre, donc `false` n'etait JAMAIS produit par
 * ce chemin et un revert atterrissait dans `unchecked`.
 *
 * Rien n'etait cache (unchecked est plus prudent que notApplicable) mais ca DILUE le seul champ qui dit
 * « va regarder toi-meme »: un lecteur qui voit « 12 non verifiees » dont 9 sont des non-jetons cesse de
 * lire la ligne, et la vraie lecture ratee s'y noie.
 *
 * `postImpl` rend ce chemin testable sans reseau — `post` utilise https.request en dur, ce qui est la
 * raison pour laquelle cette fonction exportee n'etait nommee dans aucun test. */
async function liveAllowance(chain, token, owner, spender, { attempts = 3, postImpl } = {}) {
  const p = postImpl || post;
  const rpc = RPC[String(chain).toLowerCase()];
  if (!rpc) return null;                       // chaine non cablee: non lu, jamais « rien ici »
  for (let i = 0; i < attempts; i++) {
    const res = await p(rpc, { jsonrpc: '2.0', id: 1, method: 'eth_call',
      params: [{ to: token, data: ALLOWANCE_SEL + pad(owner) + pad(spender) }, 'latest'] });
    if (res && typeof res.result === 'string' && res.result !== '0x') {
      try { return BigInt(res.result); } catch { return null; }   // forme illisible: non lu, pas zero
    }
    // '0x' = aucune donnee de retour: rien a cette adresse. Definitif, et deja traite comme false par multicall.
    if (res && res.result === '0x') return false;
    // A revert (a token without allowance(), or a non-token address) is a definitive answer, not congestion.
    if (res && res.error && !/limit|rate|busy|timeout/i.test(String(res.error.message || ''))) return false;
    await new Promise((r) => setTimeout(r, 400 * (i + 1)));
  }
  return null;                                 // tentatives epuisees (congestion): NON LU, pas « rien ici »
}
...
async function checkApprovals(chain, owner, { fromBlock = 0, maxPairs = 40 } = {}) {
  ...
  /* ⚠️ CE FICHIER SE MEFIAIT DE LA CHAINE ET FAISAIT CONFIANCE A L'APPELANT — a l'envers. Ligne ~116, le
   * `spender` LU DANS UN LOG est valide par `/^0x[0-9a-f]{40}$/`; l'`owner` FOURNI par l'utilisateur ne
   * l'etait pas. Or c'est l'entree de l'utilisateur qui peut etre une faute de frappe, un `undefined`, ou
   * un champ oublie.
   *
   * Mesure du 2026-07-29 sur la vraie surface MCP: `open_approvals` sans `owner`, et avec un `owner`
   * malforme, rendaient tous deux
   *
   *     ok: true, approvals: [], note: "STATE, not history. Every entry was confirmed by calling allowance()"
   *
   * — un balayage PROPRE ET CONFIRME pour un portefeuille jamais nomme. `pad(undefined)` produit un
   * topic, l'explorateur ne trouve rien, la liste vide devient un feu vert. Sur l'outil qui repond
   * « as-tu des autorisations ouvertes ? », c'est la pire direction possible.
   *
   * Un refus n'est PAS un resultat: il ne dit rien sur le portefeuille, et il doit le dire. */
  const o = String(owner == null ? '' : owner).toLowerCase();
  if (!/^0x[0-9a-f]{40}$/.test(o)) {
    return { ok: false, rejectedInput: true,
      reason: 'REJECTED INPUT: "' + (owner == null ? String(owner) : String(owner).slice(0, 24))
        + '" is not a well-formed address. Nothing was read, so this says NOTHING about any wallet — it is '
        + 'not an empty approval list. Pass a 0x-prefixed 40-hex address.' };
  }
  ...
  for (const l of logs) {
    ...
    // An approval to the zero address grants a permission nobody can use, so it is not a door. Spam tokens
    // emit these, and carrying them through kept the sweep permanently one pair short of "complete" — a
    // guard that can never say "everything was seen" gets ignored like one that cries wolf.
    if (/^0x0{40}$/.test(spender)) { nonsense++; continue; }
  ...
  // One batched request instead of forty sequential ones. Sequential reads left eleven to thirteen
  // allowances unread on every run — rate limiting, not a coding error — which meant the guard was blind on
  // roughly a third of its own perimeter, and an attacker's approval could sit precisely in the unread set.
  // Multicall3 collapses the batch into a single round trip. The per-pair fallback is kept for anything the
  // batch still could not resolve, so a partial batch degrades instead of hiding.
  const batched = await allowancesBatch(chain, owner, candidates);

  const live = [];
  let revoked = 0, notApplicable = nonsense;
  const unchecked = [];
  for (let i = 0; i < candidates.length; i++) {
    const c = candidates[i];
    let a = batched[i];
    if (a === null) {   // no answer at all — worth one more try on its own
      const solo = await liveAllowance(chain, c.token, owner, c.spender);
      await new Promise((r) => setTimeout(r, 120));
      a = solo;
    }
    if (a === null) { unchecked.push(c); continue; }
    if (a === 0n) { revoked++; continue; }
    if (a === false) { notApplicable++; continue; }   // the call reverted: a definitive "nothing here"
  ...
  return { ok: true, owner, chain, scanned: candidates.length, live, revoked, unlimited,
    notApplicable, unchecked: unchecked.length, uncheckedPairs: unchecked,
