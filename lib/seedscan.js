...
/**
 * Refuse to answer on an index that never came through `loadWordlist`.
 *
 * `loadWordlist` guards its own output hard — 2048 entries, unique 4-letter prefixes — but the three
 * functions that USE an index take it as a parameter, and all three are exported. Nothing forced the value
 * they received to have passed that guard, and a degraded one is silent in exactly the direction that
 * matters. Measured 2026-08-01, on a file whose only content is the all-zeros 12-word test vector written
 * as a numbered list:
 *
 *   index                       scanText          judgeRun on the run
 *   loadWordlist() .......... ['confirmed']     verdict: 'confirmed'          <- the truth
 *   new Map() ............... []                verdict: 'wordlist_run'
 *   Map with 1 of 2048 ...... []                verdict: 'wordlist_run'
 *
 * Both degraded rows are byte-for-byte what a CLEAN file produces. `scanText` returns the empty array that
 * means "no phrase here", over a file that is nothing but a phrase; `judgeRun` returns the verdict whose own
 * note reads "a partial, a transcription error ... never treated as a seed". A failure to look rendered as a
 * finding, on the one question where a false negative means the owner walks away from a seed sitting in
 * cleartext. This file already argues the point one screen down — "Caps that stay quiet are how a partial
 * scan gets read as a clean bill of health, which on this particular question is the worst possible failure"
 * — and every cap in `scanPaths` reports itself. The wordlist was the one input that did not.
 *
 * So it throws, which is this module's existing idiom for "I cannot answer this": `judgeRun` already throws
 * on an empty run rather than pointing nowhere, and `loadWordlist` throws on a corrupt list rather than
 * silently detecting less than it claims to. A caller who mishandles the index gets a stack trace instead of
 * a clean bill of health.
 *
 * The bound in the other direction is deliberate and pinned in the tests: a SANE index holding a word that is
 * genuinely off-list still returns a plain `false`. That is a real answer, not a missing one, and hardening
 * it into a throw would be the fail-closed pushed past the point where it informs.
 */
function assertUsableIndex(index, who) {
  if (!(index instanceof Map)) {
    throw new Error(who + ' requires the Map returned by loadWordlist(), got ' +
      (index === null ? 'null' : typeof index) + ' — refusing to report on a wordlist it does not have.');
  }
  if (index.size !== 2048) {
    throw new Error(who + ' requires a complete BIP-39 index of 2048 words, got ' + index.size +
 ...
 * matters, so it computes the standard rather than approximating it. Certainty is why the index is checked
 * first: without it, `false` would also be what an absent wordlist returns.
 */
function checksumValid(words, index) {
  assertUsableIndex(index, 'checksumValid');
...
function scanText(text, index, { minRun = 12 } = {}) {
  // Before the tokeniser, because a degraded index makes `index.has` false for every token: no run is ever
  // built, and the function returns the empty array that reads as "this file is clean". See assertUsableIndex.
  assertUsableIndex(index, 'scanText');
  const findings = [];
...
function judgeRun(run, index) {
  /* Exporte, donc appelable directement. Un run vide n'a AUCUNE localisation, et la seule chose que ce
   * module ne doit jamais faire est de produire un constat qui ne pointe nulle part: refuser bruyamment
   * plutot que de rendre un objet dont `line` vaut undefined. */
  if (!Array.isArray(run) || run.length === 0) throw new Error('judgeRun requires a non-empty run of tokens');
  // Same reason, other input: with a degraded index every checksum test fails and the run is published as
  // 'wordlist_run' — "a partial, a transcription error" — over a genuine phrase. See assertUsableIndex.
  assertUsableIndex(index, 'judgeRun');
  const words = run.map((t) => t.w);
  const runLine = run[0].line;
  const runEndLine = run[run.length - 1].line;
  ...
  if (best) {
    /* LA LOCALISATION EST LE PRODUIT — ce module n'imprime JAMAIS la phrase, il dit ou aller regarder. Elle
     * doit donc etre celle de la PHRASE, pas celle du run qui la contient. Elle etait celle du run: mesure du
     * 2026-07-29, une phrase de 12 mots aux lignes 2-13 suivie de 40 mots-liste etait annoncee « spanning
     * lines 1-53 ». Le champ ne variait pas avec la position de la phrase — il decrivait la forme du run,
     * pendant que `words: 12` a cote le contredisait. Envoyer quelqu'un lire 53 lignes pour 12 mots est
     * exactement l'inverse du service rendu. */
    const first = run[best.start];
    const last = run[best.start + best.len - 1];
    const line = first.line, endLine = last.line;
    const span = endLine > line ? ' spanning lines ' + line + '-' + endLine : ' on line ' + line;
    /* La recherche s'arrete au PREMIER checksum valide, et un run peut en porter plusieurs: deux phrases
     * ecrites l'une sous l'autre ne sont separees par aucun non-mot, donc elles forment UN seul run et la
     * seconde n'est pas rapportee (mesure le 2026-07-29). Reprendre la recherche apres la premiere ferait
     * revenir la multiplicite que la borne OFFSET_SLACK existe pour empecher — donc on garde la borne et on
     * DIT la limite, comme chaque autre plafond de ce fichier. */
    const debordement = best.start > 0 || best.start + best.len < run.length
      ? ' It sits inside an unbroken run of ' + words.length + ' wordlist words (lines ' + runLine + '-'
        + runEndLine + '). The search stops at the first valid checksum, so a SECOND phrase inside that run '
        + 'would not be reported here: read the whole run.'
      : '';
    return { line, endLine, words: best.len, runLine, runEndLine, runWords: words.length, verdict: 'confirmed',
      note: 'A valid BIP-39 checksum' + span + '. This is a recovery phrase, in cleartext, at this location.'
        + debordement };
  }
  const span = runEndLine > runLine ? ' spanning lines ' + runLine + '-' + runEndLine : '';
  return { line: runLine, endLine: runEndLine, words: words.length,
    runLine, runEndLine, runWords: words.length, verdict: 'wordlist_run',
...
/**
 * ⚠️ UNE LISTE DE CHEMINS INUTILISABLE N'EST PAS UNE LISTE VIDE — mesure du 2026-07-30.
 *
 * `for (const root of paths)` accepte tout ce qui est iterable, et une CHAINE l'est: passer
 * "C:\\Users\\moi\\notes" au lieu de ["C:\\Users\\moi\\notes"] ne jette pas, ne previent pas, et decoupe
 * l'argument en autant de chemins d'UN caractere. Deux consequences mesurees:
 *   · le bulletin annonce « 22 path(s) were given » — un compteur fabrique, pour un seul chemin demande;
 *   · sous Windows, `fs.existsSync('\\')` rend TRUE, donc le balayage part sur la RACINE DU DISQUE a
 *     profondeur 6. La premiere sonde ecrite pour mesurer ce defaut a tourne plus de 120 secondes.
 * Le resultat porte alors sur un endroit que personne n'a nomme, exactement le defaut corrige le matin
 * meme dans `defaultPaths` (`|| '.'`).
 *
 * On refuse BRUYAMMENT plutot que de deviner, comme `judgeRun` sur un run vide: le seul geste qu'aucun
 * de ces deux modules ne doit faire est de rendre un constat rassurant sur un endroit qu'on n'a pas
 * regarde. Une liste vide, elle, reste VALIDE — c'est ce que `defaultPaths` rend quand aucune racine
 * n'est resolue, et `scanPaths` sait deja la traduire en `not_scanned`.
 */
function exigeListeChemins(paths, champ = 'paths') {
  if (!Array.isArray(paths)) {
    throw new TypeError(champ + ' must be an ARRAY of folder paths, got ' + (paths === null ? 'null' : typeof paths)
      + '. A bare string is iterable and would be read as one path per character, scanning places you never '
      + 'named. Nothing was scanned.');
  }
  const mauvais = paths.filter((p) => typeof p !== 'string' || !p.trim());
  if (mauvais.length) {
    throw new TypeError(champ + ' contains ' + mauvais.length + ' entr' + (mauvais.length > 1 ? 'ies' : 'y')
      + ' that are not non-empty strings. An empty or non-string path cannot be scanned, and skipping it '
      + 'quietly would count as coverage. Nothing was scanned.');
  }
  return paths;
}

function scanPaths(paths, index, { maxFiles = 20000 } = {}) {
  exigeListeChemins(paths);
  const fs = require('node:fs');
  const path = require('node:path');
  const findings = [];
  /* `absent` est SEPARE d'`unreadable`: un dossier absent de cette machine n'a rien a lire, alors qu'un
   * dossier qui existe et refuse de s'ouvrir est une couverture perdue. Voir la note sur `complete`. */
  const skipped = { tooBig: 0, unreadable: 0, notTextual: 0, overFileCap: 0, absent: 0 };
  ...
  for (const root of paths) {
    try { if (!fs.existsSync(root)) { skipped.absent++; continue; } } catch { skipped.unreadable++; continue; }
    walk(root, 0);
  }
  ...
  /* LA PORTEE VOULUE N'EST PAS UNE COUVERTURE PERDUE. `complete` valait `totalSkipped === 0`, ce qui
   * additionnait les images qu on ne SAIT PAS lire (exclusion voulue) et ce qui a BLOQUE une lecture
   * prevue. Mesure du 2026-07-28: un dossier propre avec 2 images et le meme dossier plus un chemin
   * inexistant rendaient TOUS DEUX complete:false. Comme tout dossier Documents reel contient des images,
   * le drapeau valait false 100 % du temps — un chiffre qui ne change jamais n observe pas, il affirme. */
  const bloquees = skipped.unreadable + skipped.tooBig + skipped.overFileCap;
  /* ⚠️ AUCUNE LECTURE N'EST PAS UNE ABSENCE DE PHRASE — troisieme etat ajoute le 2026-07-30.
   *
   * Mesure avant correctif, les six chemins par defaut absents de la machine (le cas d'un `seed_exposure`
   * appele sans argument sur une machine qui range ses notes ailleurs, ou d'un HOME non exporte):
   ...
   * soit le bulletin de sante le PLUS rassurant que ce module sache produire, rendu par un balayage qui a
   * ouvert ZERO fichier. Le compteur `absent: 6` etait bien la, mais la phrase juste a cote le contredisait:
   * « nothing blocked a read » n'etait vrai que parce qu'il n'y avait rien a bloquer. Et un dossier reel
   * vide, un chemin inexistant et une liste de chemins vide rendaient tous les trois le meme mot.
   *
   * `nothing_found` est une affirmation sur la phrase; elle exige d'avoir lu quelque chose. `not_scanned`
   * n'affirme rien sur la phrase — il dit qu'on n'a pas regarde, ce qui est la seule chose vraie ici. */
  const rienLu = scanned === 0;
  return {
    scanned, bytes, findings, confirmed: confirmed.length, skipped,
    complete: bloquees === 0 && !rienLu,
    coverage: rienLu
      ? 'none — NOT ONE FILE WAS OPENED, so this run says nothing at all about your phrase. '
        + paths.length + ' path(s) were given, ' + skipped.absent + ' of them are not on this machine, '
        + skipped.notTextual + ' file(s) found were non-textual, and ' + bloquees + ' read(s) were blocked. '
        + 'If you expected these folders to exist, that is the finding. Pass the paths you mean explicitly.'
      : bloquees === 0
        ? 'as_intended — nothing blocked a read. ' + skipped.notTextual + ' non-textual file(s) were never '
          + 'readable by design and ' + skipped.absent + ' path(s) are not on this machine at all.'
        : 'reduced — ' + bloquees + ' file(s) or folder(s) we INTENDED to read could not be: '
          + skipped.unreadable + ' unreadable, ' + skipped.tooBig + ' over the size limit, '
          + skipped.overFileCap + ' past the file cap. A phrase could sit in any of them.',
    verdict: confirmed.length ? 'exposed' : rienLu ? 'not_scanned'
      : ambiguous.length ? 'inconclusive' : 'nothing_found',
    disclosure: 'This never says "safe". A confirmed hit is a FACT — the BIP-39 checksum validated — and the ' +
      ...
      'WHAT WAS READ — and "not_scanned" means nothing was read at all, which is not a result about your ' +
      'phrase. It cannot see images, PDFs, password managers, browser storage, encrypted archives, ' +
      'deleted-but-recoverable files, or anything outside the paths given. Read `coverage`: it says in words ' +
      'whether anything BLOCKED a read, or whether the only gaps are ones this scan never claimed to cover.',
  };
}
 ...
 *
 * ⚠️ PAS DE REPLI SUR '.'. Mesure du 2026-07-30: sans USERPROFILE ni HOME, `|| '.'` rendait
 * ["Documents","Desktop","Downloads","OneDrive\\Documents","OneDrive\\Desktop","Notes"] — six chemins
 * RELATIFS au repertoire de travail du processus hote, alors qu'aucune racine n'avait ete resolue. Au mieux
 * ils sont tous absents et le balayage rend un bulletin vide (voir `not_scanned` dans `scanPaths`); au pire
 * un dossier `Documents` traine dans ce repertoire-la et on balaie le MAUVAIS dossier en le rapportant comme
 * celui de l'utilisateur. Un environnement depouille (service, conteneur, hote MCP qui n'exporte pas HOME)
 * suffit. Une racine inconnue rend donc une liste VIDE — c'est un refus de deviner, et les deux consommateurs
 * (`scanPaths`, `scanKeyPaths`) le traduisent en « on n'a pas regarde », jamais en « rien trouve ».
 */
...
module.exports = { loadWordlist, checksumValid, scanText, judgeRun, scanPaths, defaultPaths,
  exigeListeChemins, assertUsableIndex, VALID_LENGTHS };
