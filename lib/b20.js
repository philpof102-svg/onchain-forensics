 ...
 *
 * ⚠️ CORRECTED 2026-07-27, hours after first writing it. This comment originally claimed the marker "cannot
 * be forged because EIP-3541 forbids deploying any runtime code beginning with 0xEF". That justification is
 * WRONG and the error was load-bearing:
 *
 *   - EIP-3541 constrains only the CREATE path. It is true that a state-override CREATE with runtime `0xEF`
 *     returns the zero address while an identical CREATE with `0x00` succeeds — that was measured — but it
 *     does not follow that no account can ever carry 0xEF-prefixed code.
 *   - **EIP-7702 routinely puts 0xEF-prefixed code on ordinary EOAs.** A delegation indicator is exactly
 *     `0xef01 00 || <20-byte address>` — 23 bytes. Corroborated on Base by reading the bytecode of
 *     0xdfac5e91fd27451123133da9f71d68c0b247b4de, which contains an explicit validator for that shape:
 *     it checks `length == 0x17`, then bytes `ef`, `01`, `00`, then reads the address at offset 0x23.
 *     A contract that validates the pattern exists because the pattern is common.
 *
 * WHAT ACTUALLY GUARANTEES THE CHECK is the EXACT EQUALITY, and nothing else. A native B20 carries ONE byte;
 * a 7702 delegation carries twenty-three. They are distinguishable only if the comparison stays exact.
 *
 * ⛔ THEREFORE: never relax this to `startsWith('0xef')` or to a length bound. Measured the same day —
 * `'0xef0100dfac…'.startsWith('0xef')` is `true`, so that one-character relaxation would classify EVERY
 * 7702-delegated EOA sitting at a ground b200 vanity address as an authentic B20. A regression test in
 * test/b20.test.js pins this; if you are here because that test is in your way, it is doing its job.
 *
