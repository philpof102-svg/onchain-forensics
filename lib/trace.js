...
/**
 * ⚠️ THE CHECKSUM WAS DROPPED WITHOUT BEING READ — `h.slice(0, -8)` threw away the four bytes whose only
 * job is to catch a mistyped address, and returned the rest as if it were an answer.
 *
 * Measured 2026-07-29, changing ONE character of a valid address:
 *
 *   original          41a614f803b6fd780986a42c78ec9c7f77e6ded13c
 *   char  5 changed   41a614f813fa29ceaab4a35d0a72d87d6117cfaac7   <- a DIFFERENT wallet, returned silently
 *   char 15 changed   41a614f803b6fd780986a42f3144dd7b955126dc24   <- a DIFFERENT wallet, returned silently
 *   char 33 changed   41a614f803b6fd780986a42c78ec9c7f77e6ded13c   <- same body: proof the last 4 bytes
 *                                                                     were never even looked at
 *
 * On a module whose output NAMES A WALLET in a theft trace, a typo, a bad OCR or a truncated paste turned
 * into a confident hex address belonging to somebody else. That is the worst version of this repository's
 * standing rule — an address is copied, never recalled, and never reconstructed from a value that failed
 * to verify. `hexToTron` right above computes a real double-SHA256 checksum, so the two are now inverses
 * that agree; anything that does not verify is refused rather than approximated.
 */
function tronToHex(addr) {
  ...
  // A base58check payload here is exactly 25 bytes: 0x41 prefix + 20 address + 4 checksum. Pad rather than
  // reject on a short hex string — a leading zero byte is lost by toString(16), not absent from the address.
  const h = n.toString(16).padStart(50, '0');
  if (h.length !== 50) return null;
  const body = h.slice(0, -8);
  const claimed = h.slice(-8);
  const actual = crypto.createHash('sha256')
    .update(crypto.createHash('sha256').update(Buffer.from(body, 'hex')).digest())
    .digest().subarray(0, 4).toString('hex');
  if (actual !== claimed) return null;   // a mistyped address is NOT a different address, it is no address
  return body;
}
...
async function followTron(address, { maxHops = 3, lireJson } = {}) {
  ...
  for (let hop = 0; hop < maxHops && current; hop++) {
    ...
    /* `tronToHex` now REFUSES an address whose checksum does not verify, so this can be null where it never
     * was before — and a new null that a consumer misreads is how a fix becomes a defect. Left unhandled,
     * `(ownHex || '')` below compares every transfer against the empty string, so nothing matches, every
     * movement reads `in`, `outs` is empty and the loop stops: THE FALSE TERMINUS documented twenty lines
     * above, rebuilt through a different door. Direction is therefore only claimed when the address decoded. */
    const ownHex = tronToHex(current);
    const adresseLue = ownHex !== null;
    ...
    for (const t of items) {
      ...
      flow.push({ ts: new Date(t.block_timestamp).toISOString(), amount: amt,
        ...
        direction: !adresseLue ? 'unknown'
          : (p.owner_address || '').toLowerCase() === ownHex.toLowerCase() ? 'out' : 'in' });
    }
    ...
    hops.push({ address: current,
      ...
      accountRead: compteLu, transactionsRead: txsLues, addressDecoded: adresseLue,
    ...
    if (!adresseLue) { arret = 'address_unreadable'; break; }   // sans adresse decodee, aucune direction n'est calculable
  ...
  const RAISONS = {
    ...
    address_unreadable: 'THE TRAIL WAS CUT BY AN ADDRESS THAT DOES NOT VERIFY, not by the funds stopping. '
      + 'The last address below failed its base58check checksum, so it is not a valid TRON address — most '
      + 'likely a typo, a bad paste or a truncated copy. Nothing about it was concluded, and nothing should '
      + 'be: the address one character away belongs to someone else entirely.',
  };
  return { ok: true, hops,
